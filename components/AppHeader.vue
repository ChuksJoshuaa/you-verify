  ...
  <div class="ml-[3em] h-[5rem] app-container relative w-full px-0 md:px-2 border-b-1 border-gray-100">
    <div class="py-2 select-top z-10">
      <select name="select" v-model="selectedValue" class="hide-search z-5 select-body outline-none border border-gray-100 text-md capitalize py-1 rounded cursor-pointer bg-gray-100 text-gray-900">
        ...
        <option :value="option.id" class=" outline-none capitalize bg-white text-gray-900 text-lg p-2"  v-for="option in options" :key="option.id" >{{ option.label }}</option>
      </select>
      <div class="relative">
        <input v-model="inputValue" type="text" tabindex="1" class="hide-search w-full input-box bg-gray-100 border-l-1 border-gray-300 p-2 outline-none" placeholder="Search for anything here" />
        <button class="absolute right-[1em] top-[13px] hide-search ">
          <Icon @click="log" name="bi:search" class="cursor-pointer font-bold text-gray-600 text-lg md:text-2xl"></Icon>
        </button>
      </div>
      <div class="px-0 md:px-3 profile-check w-full">
        <div class="relative mr-3">
          <Icon name="bi:bell-fill"
            class="ml-2 md:ml-[1em] font-bold cursor-pointer text-[#22A6B5] h-[42px] w-[42px] md:h-[62px] md:w-[62px] p-2 md:p-4 shadow-lg bg-main-bg rounded-lg"></Icon>
            <h4 class="absolute top-0 right-0 text-sm md:text-xl text-center font-bold text-gray-50 rounded-full bg-red-600 h-[20px] w-[20px] md:h-[30px] md:w-[30px]">2</h4>
         </div>
...
const options = useState('options', () => data)
const selectedValue = useState('selectedValue', () => "")
const inputValue = useState('inputValue', () => "")

function log() {
  const selectValue = `selectedValue: ${selectedValue.value}`
  const selectInput = `inputValue: ${inputValue.value}`

  if (!selectedValue.value || !inputValue.value) {
